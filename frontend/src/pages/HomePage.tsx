    ...
    <div className="bg-[#131313] text-[#e5e2e1] min-h-screen">
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-20 px-6 md:px-8 overflow-hidden noise-bg">
        ...
        <div className="relative z-10 max-w-6xl w-full flex flex-col md:flex-row items-center gap-12 md:gap-16">
          <div className="flex-1 text-center md:text-left">
            ...
            <h1 className="font-headline text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-extrabold tracking-tighter text-[#e5e2e1] mb-6 leading-none">Building the{" "}<span className="text-[#53e076] italic">Impossible</span></h1>
            <div className="flex items-center justify-center md:justify-start gap-4 mb-6"><div className="h-[2px] w-10 bg-[#1db954]" /><p className="font-label text-lg text-[#1db954] font-medium tracking-tight">{heroTagline}</p></div>
            <p className="text-[#bccbb9] text-base md:text-lg max-w-xl mb-8 leading-relaxed font-body mx-auto md:mx-0">{heroBio}</p>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-40"><span className="font-label text-[10px] tracking-widest uppercase">Scroll</span><div className="w-px h-8 bg-gradient-to-b from-[#e5e2e1] to-transparent" /></div>
      </section>

      <section className="py-20 px-6 md:px-8 relative" aria-label="Statistik">
        <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-6">
          {[{value:"4+",label:"Years Experience"},{value:"8+",label:"High-End Projects"},{value:"3.45",label:"GPA Excellence"}].map(({value,label}) => (
            <div key={label} className="bg-[#1c1b1b] p-10 md:p-12 rounded-3xl flex flex-col items-center text-center shadow-2xl transition-transform hover:-translate-y-2 group">
      ...
      <section className="py-20 px-6 md:px-8 max-w-7xl mx-auto" aria-label="Featured Projects">
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div><h2 className="font-headline text-4xl md:text-6xl font-extrabold tracking-tighter mb-3 text-[#e5e2e1]">Curated <span className="text-[#1db954]">Orbits</span></h2><p className="text-[#bccbb9] font-body text-base max-w-md">A selection of technical explorations and architectural achievements.</p></div>
      ...
      <section className="py-28 px-6 md:px-8 overflow-hidden bg-[#0e0e0e] relative" aria-label="Skills">
        <div className="max-w-6xl mx-auto text-center mb-16"><h2 className="font-label text-xs uppercase tracking-[0.4em] text-[#53e076] mb-4">Technological Core</h2><h3 className="font-headline text-4xl md:text-5xl font-bold tracking-tighter text-[#e5e2e1]">The Arsenal</h3></div>
        <div className="flex flex-wrap justify-center gap-4 max-w-5xl mx-auto">
          {[{icon:"javascript",label:"JavaScript"},{icon:"data_object",label:"TypeScript"},{icon:"token",label:"React / Next.js"},{icon:"storage",label:"PostgreSQL"},{icon:"cloud",label:"Docker / DevOps"},{icon:"palette",label:"Tailwind CSS"},{icon:"animation",label:"Framer Motion"},{icon:"api",label:"Elysia.js"},{icon:"code",label:"Laravel / PHP"},{icon:"database",label:"Drizzle ORM"}].map(({icon,label}) => (
            <div key={label} className="flex items-center gap-3 bg-[#2a2a2a]/40 border border-[#3d4a3d]/10 px-5 py-3 rounded-full glass-card hover:border-[#53e076]/50 hover:shadow-[0_0_20px_rgba(83,224,118,0.1)] transition-all cursor-default group">
              <span className="material-symbols-outlined text-[#53e076] text-xl group-hover:scale-110 transition-transform" aria-hidden="true">{icon}</span>
      ...
      <section className="py-28 px-6 md:px-8 max-w-4xl mx-auto" aria-label="Experience Timeline">
        <div className="mb-16"><h2 className="font-headline text-3xl md:text-4xl font-extrabold mb-3 text-[#e5e2e1]">Journey</h2><div className="h-1 w-16 bg-[#53e076] rounded-full" /></div>
        <div className="space-y-12 md:space-y-16 relative before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-[2px] before:bg-[#3d4a3d]/40">
          {[{period:"2025 — 2026",role:"Frontend Developer",company:"PT Technova Solusi Integrasi",desc:"Developing ReactJS monitoring dashboard with OpenStreetMap & ML integration.",active:true},{period:"2025 — 2026",role:"Full Stack Developer Intern",company:"PT Kimia Farma Tbk",desc:"Built internal HRIS using Laravel + PostgreSQL + Redis caching.",active:false},{period:"2021 — Present",role:"Bachelor of Informatics",company:"ITERA — Institut Teknologi Sumatera",desc:"GPA 3.45. Specialized in distributed systems and human-computer interaction.",active:false,icon:"school"}].map(({period,role,company,desc,active,icon}) => (
      ...
      <section className="py-20 px-6 md:px-8 max-w-6xl mx-auto" aria-label="Latest posts">
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div><h2 className="font-headline text-4xl md:text-5xl font-extrabold tracking-tighter text-[#e5e2e1]">Latest <span className="text-[#1db954]">Notes</span></h2><p className="text-[#bccbb9] text-sm mt-2">Technical writing &amp; thought experiments from the orbit.</p></div>
          <Link to="/posts" className="font-label text-[#53e076] text-sm uppercase tracking-widest font-bold flex items-center gap-2 hover:text-[#72fe8f] transition-colors">All Posts <span className="material-symbols-outlined text-base" aria-hidden="true">arrow_outward</span></Link>
        </div>
        {postsLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {latestPosts.map(post => (
              <Link key={post.id} to={`/news/${post.slug}`} className="group bg-[#1c1b1b] rounded-3xl p-6 border border-[#3d4a3d]/10 hover:border-[#53e076]/30 transition-all hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#53e076]">
